...
mod1 = neuralnet(formula = form,data=train2,hidden = c(9,5),threshold = 0.01, stepmax = 50*epoch,
...
M1= mmetric(train2$Close.Price, mod2$net.result[[1]][[1]], c("SSE","RMSE","ME"))
...
m3=mmetric(test2$Close.Price, mod2test$net.result[,1],c("SSE","RMSE","ME"))
...
for (i in th) {
  mod4=neuralnet(form,algorithm = "rprop+", threshold = i, stepmax = 50*epoch,
                 data=train1, hidden = c(9,5), linear.output = T,
                 rep = 20)
  best=as.integer(which.min(mod4$result.matrix[c("error"),]))
  mod4test=compute(mod4, test1[,-c(5)],rep = best)
  M6=mmetric(test1$Close.Price,mod4test$net.result[,1],c("RMSE"))
  Mtest=c(Mtest,M6)
...
mod5=neuralnet(mf,algorithm = "rprop+", threshold = 0.08, stepmax = 30*epoch,
               data = train1, hidden = c(9), linear.output = T,
               rep=20)
